          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                setTestResult(`Status: ${res.status} — ${text.slice(0, 200)}`);
              } catch (e) {
                setTestResult(`FAILED: ${e.message}`);
              }
            }}
            style={{ marginTop: 10, background: COLORS.greige, color: COLORS.white, border: "none", borderRadius: 6, padding: "8px 18px", fontSize: 12, fontWeight: 600, cursor: "pointer", width: "100%" }}>
            🔧 Test Worker Connection
          </button>
          {testResult && (
            <div style={{ marginTop: 8, background: COLORS.lightGrey, border: `1px solid ${COLORS.midGrey}`, borderRadius: 6, padding: "10px 14px", fontSize: 12, color: COLORS.dark, wordBreak: "break-all" }}>
              {testResult}
            </div>
          )}
